...
for x in range(number_of_messages):
   if my_rank == 0:
      #print(f"I am {my_rank} before send ping " + str(buffer))
      if x == 0:
         start = MPI.Wtime()
      comm_world.send(buffer, dest=1, tag=17)
      buffer = comm_world.recv(source=1, tag=23, status=status)
      #print(f"I am {my_rank} after  recv pong " + str(buffer))
      buffer = buffer + 1
      if(x==number_of_messages-1):
         finish = MPI.Wtime()
   elif my_rank == 1:
      buffer = comm_world.recv(source=0, tag=17, status=status)
      #print(f"I am {my_rank} after  recv ping " + str(buffer))
      #print(f"I am {my_rank} before send pong " + str(buffer))
